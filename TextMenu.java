...
public class TextMenu extends JMenu {
	...
	public TextMenu(DrawPanel owner) {
		...
		final JMenuItem lineHeight = new JMenuItem("Set Height...");
		lineHeight.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				double currentHeight = panel.getTextItem().getLineHeightMultiplier();
				String str = JOptionPane.showInputDialog(panel, "What height do you want to use?", currentHeight);
				if(str != null && str.trim().length() > 0) {
					try {
						int newHeight = Integer.parseInt(str.trim());
						panel.getTextItem().setLineHeightMultiplier(newHeight);
						panel.repaint();
					} catch(Exception e) {
						JOptionPane.showMessageDialog(panel, str + "is not a valid text height." + " Please enter a positive number");
					}
				}
			}
		});
	...
	private JMenu makeFontNameSubmenu() {
		...
		/**
		 * Menu with all the available text justification
		 */
		
		private JMenu makeJustifyMenu() {
			ActionListener JustifyActionLeft = new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					panel.getTextItem().setJustify(TextItem.LEFT);
					panel.repaint();
				}
			};
			ActionListener JustifyActionRight = new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
					panel.getTextItem().setJustify(TextItem.RIGHT);
					panel.repaint();
				}
			};
			ActionListener JustifyActionCenter = new ActionListener() {
				public void actionPerformed(ActionEvent evt) {
			...
			left.addActionListener(JustifyActionLeft);
			buttonGroup.add(left);
			menu.add(left);
			JRadioButtonMenuItem right = new JRadioButtonMenuItem("Right");
			right.addActionListener(JustifyActionRight);
			buttonGroup.add(right);
			menu.add(right);
			right.setSelected(true);// default justification
			JRadioButtonMenuItem center = new JRadioButtonMenuItem("Center");
			center.addActionListener(JustifyActionCenter);
